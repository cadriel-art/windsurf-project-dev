function getConfig() {
  ...
  if (window.SiteTemplate?.loadConfig)
    return window.SiteTemplate.loadConfig({ mode });
  return {
...
function renderMenuFromConfig(config, mode) {
  ...
  let items = Array.isArray(config?.menu?.primary)
    ? [...config.menu.primary]
    : [];
  if (!items.length) {
    items = [
  ...
  function getRank(item, idx) {
    ...
    if (slug === 'home' || href === './?page=home' || href === '#home')
      return 0;
...
function resolveCapabilities(config) {
  const source =
    Array.isArray(config?.capabilities?.items) &&
    config.capabilities.items.length
      ? config.capabilities.items
      : FALLBACK_CAPABILITIES_ITEMS;
  return source.map((item, idx) => {
    const fallback =
      FALLBACK_CAPABILITIES_ITEMS[idx % FALLBACK_CAPABILITIES_ITEMS.length];
    return {
  ...
  function update(now) {
    ...
    state.particles.forEach((p) => {
      if (state.phase === 'dissolve') {
        ...
        const swirl = 0.7;
        p.vx +=
          tangX * swirl * 0.012 + windX + Math.sin((now + p.seed) / 420) * 0.01;
        p.vy +=
          tangY * swirl * 0.012 + windY + Math.cos((now + p.seed) / 420) * 0.01;
        p.vx *= 0.992;
        p.vy *= 0.992;
      }
...
function ensureHomeAboutSection(config) {
  ...
  const aboutPage = pages.find(
    (p) => (p?.slug || '').toLowerCase() === 'about',
  );
  const blocks = sanitizeBlocks(
    Array.isArray(aboutPage?.blocks) ? aboutPage.blocks : [],
  );
  ...
  header.innerHTML =
    '<span class="section-tag">about</span><h2 class="section-title">About</h2>';
  ...
  const aboutCardConfig =
    aboutPage && typeof aboutPage.aboutCard === 'object' && aboutPage.aboutCard
      ? aboutPage.aboutCard
  ...
  aboutType.textContent =
    typeof aboutCardConfig?.meta === 'string'
      ? aboutCardConfig.meta
      : 'Product-minded developer • Sci-fi UI systems';
  const aboutTitle = document.createElement('h3');
  aboutTitle.className = 'card-title';
  aboutTitle.textContent =
    typeof aboutCardConfig?.title === 'string' && aboutCardConfig.title.trim()
  ...
  const rich = blocks.find(
    (b) =>
      b?.type === 'richText' && typeof b?.html === 'string' && b.html.trim(),
  );
  if (rich) {
    const content = document.createElement('div');
    content.className = 'about-rich';
    content.innerHTML = sanitizeHTML(String(rich.html));
...
function renderPageFromConfig(config) {
  ...
  const pages = Array.isArray(config?.pages) ? config.pages : [];
  const page =
    pages.find((p) => (p?.slug || '').toLowerCase() === slug) || pages[0];
  ...
  blocks.forEach((block) => {
    if (block?.type === 'richText') {
      ...
      inner.innerHTML = sanitizeHTML(block?.html || '');
  ...
  if (slug === 'about') {
    ...
    // About card panel
    const aboutCardConfig =
      page && typeof page.aboutCard === 'object' && page.aboutCard
        ? page.aboutCard
        : config && typeof config.aboutCard === 'object' && config.aboutCard
          ? config.aboutCard
          : {};
...
function buildAboutCard(aboutCardConfig) {
  const base =
    typeof aboutCardConfig?.base === 'string'
      ? aboutCardConfig.base
      : 'electric';
  const meta =
    typeof aboutCardConfig?.meta === 'string'
      ? aboutCardConfig.meta
      : 'Product-minded developer • Sci-fi UI systems';
  const body =
    typeof aboutCardConfig?.body === 'string' && aboutCardConfig.body.trim()
  ...
  description.textContent =
    typeof aboutCardConfig?.description === 'string' &&
    aboutCardConfig.description.trim()
...
function appendAboutHighlights(section, aboutCardConfig) {
  const highlights = Array.isArray(aboutCardConfig?.highlights)
    ? aboutCardConfig.highlights
    : [];
...
function updateDomFromConfig(config) {
  ...
  const metaTitle =
    seo?.metaTitle || config?.brand?.siteTitle || document.title;
  ...
  if (seo?.metaDescription)
    upsertMetaByProperty('og:description', seo.metaDescription);
  ...
  if (typing && config?.hero?.typingText)
    typing.textContent = config.hero.typingText;

  const heroDesc = document.querySelector('.hero-description');
  if (heroDesc && typeof config?.hero?.description === 'string')
    heroDesc.textContent = config.hero.description;

  const heroCta = document.querySelector('.hero .cta-btn');
  if (heroCta) {
    if (config?.hero?.ctaText) heroCta.textContent = config.hero.ctaText;
    if (config?.hero?.ctaHref)
      heroCta.setAttribute('href', config.hero.ctaHref);
  }
  ...
  if (contactSubtitle && config?.contact?.subtitle)
    contactSubtitle.textContent = config.contact.subtitle;
...
function renderCardsFromConfig(config) {
  ...
  const elements =
    Array.isArray(config?.elements) && config.elements.length
      ? config.elements
...
function buildCard(project) {
  ...
  glowElectric.append(
    el('div', 'glow-orb glow-orb-1'),
    el('div', 'glow-orb glow-orb-2'),
    el('div', 'glow-orb glow-orb-3'),
  );
  ...
  backMeta.textContent =
    `${project.category || ''}${project.appType ? ' • ' + project.appType : ''}`.trim();
...
function cycleElements(cards, dots, elements) {
  window.__currentElementIndex =
    (window.__currentElementIndex + 1) % elements.length;
  const base = window.__currentElementIndex;
...
function startCycle(cards, dots, elements) {
  window.__cycleInterval = window.setInterval(
    () => cycleElements(cards, dots, elements),
    5000,
  );
}
...
function initCursor() {
  ...
  const interactive = document.querySelectorAll(
    'a, button, .elemental-card, .nav-link, .element-dot',
  );
